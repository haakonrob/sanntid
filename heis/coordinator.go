...
const (
	...
	UP            = int(heis.BUTTON_CALL_UP)
	DOWN          = int(heis.BUTTON_CALL_DOWN)
	COMMAND       = int(heis.BUTTON_COMMAND)
)
...
func main() {
	...
	for {
		...
		case msg := <-incomingCh:
			...
			default:
				if globalOrdersAvailable(){
					changesMade = true
					scoreAvailableOrders()	
				}
			}
		...
		case newOrder := <-orderChan:
			...
			case true && newOrder.OrderType != COMMAND:
				oldOrders := GlobalOrders
				addNewGlobalOrder(newOrder)
				if oldOrders.SenderId = GlobalOrders.SendId; oldOrders == GlobalOrders {
					changesMade = true
				}
				// fsm will be updated when packet comes around again
			case false || newOrder.OrderType == COMMAND:
				addNewLocalOrder(newOrder)
				fsmChan <- LocalOrders
			}
		...
		case completedOrders := <-fsmChan:
			...
			case true:
				oldOrders := GlobalOrders
				completeGlobalOrders(completedOrders)
				if oldOrders.SenderId = GlobalOrders.SendId; oldOrders == GlobalOrders {
					changesMade = true
				}

			case false:
				completeLocalOrders(completedOrders)
				fsmChan <- LocalOrders
			}
...
func addNewLocalOrder(order heis.Order) {
...
func addNewGlobalOrder(order heis.Order) {
	ordertype := order.OrderType
	floor := order.Floor
	stamp := time.Now()

	switch ordertype {
	case ordertype == UP || ordertype == DOWN :
...
type orderLogic func(int, int)(bool)

//Iterates over specified intevals with a function, 
//returns true if a single iteration resulted in true
func orderIterate( oEnd int, fEnd int, orderFun orderLogic ){
	result := false
	for o := UP; o <= oEnd; o++ {
		for floor := 0; floor < fEnd; floor++ {
			if orderFun(){
				result = true
			}
		}
...
func globalOrdersAvailable(ordertype int, floor int){
	if GlobalOrders[order][floor]{
		return true
	}
	return false
}
...
//3 x N_FLOORS
func completeLocalOrders(ordertype int, floor int) {
...
// 3 x N_FLOORS
func completeGlobalOrders(o int, floor int)(true) {
	pending := newlocalOrders.Pending[ordertype][floor]
	completed := newlocalOrders.Completed[ordertype][floor]
...
//3 x N_FLOORS
func setLight(ordertype int, floor int) {
	switch ordertype {
...
//2 x N_FLOORS
func scoreAvailableOrders(ordertype int, floor int) {
...
func takeGlobalOrder(ordertype int, floor int) {
	changesMade := false
...
//2 x N_FLOORS
func mergeOrders(ordertype int, floor int) {
